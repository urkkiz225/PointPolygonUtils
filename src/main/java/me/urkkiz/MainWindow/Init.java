...
public class Init extends JFrame{
    ...
    public static void InitWindow(){
        ...
        GeneralInfo.setLocation(Math.abs(Toolkit.getDefaultToolkit().getScreenSize().width-600), Math.abs(Toolkit.getDefaultToolkit().getScreenSize().height-100)-260);
        GeneralInfo.setOpaque(true);
        SelectedPolygon.setBounds(frame.getWidth(),frame.getHeight(),600,100);
        SelectedPolygon.setFont(new Font(null, Font.PLAIN,24));
        SelectedPolygon.setLocation(Math.abs(Toolkit.getDefaultToolkit().getScreenSize().width-600), Math.abs(Toolkit.getDefaultToolkit().getScreenSize().height-100)-320);
        SelectedPolygon.setOpaque(true);
        AmountOfCollisions.setBounds(frame.getWidth(),frame.getHeight(),600,100);
        AmountOfCollisions.setFont(new Font(null, Font.PLAIN,24));
        AmountOfCollisions.setLocation(Math.abs(Toolkit.getDefaultToolkit().getScreenSize().width-600), Math.abs(Toolkit.getDefaultToolkit().getScreenSize().height-100)-380);
        AmountOfCollisions.setOpaque(true);
        label.setBounds(frame.getWidth(),frame.getHeight(),300,100);
        label.setFont(new Font(null,Font.BOLD,25));
        frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        frame.setTitle(new String[]{"polygon point epic application","\"Polygon polygon, polygon!\" -Polygon", "The usurper is waiting...", "Do these two lines really intersect?", "I love concave polygons!!1!!","Minecraft 1.8.9", "ULTRAKILL"}[random.nextInt(0,6)]);
