        ...
        if (Init.GeneralInfo.isVisible()) {
            ...
            if (PolygonHolder.TempPolygon.size()!=0) {
                for (int j = 0; j < PolygonHolder.TempPolygon.size()-1; j++) {
                    ...
                    System.out.println(PolygonHolder.TempPolygon.size());
                    ...
                    if(j+2==PolygonHolder.TempPolygon.size()) g.draw((Math.abs(PolygonHolder.TempPolygon.get(j+1)[0]-PolygonHolder.TempPolygon.get(0)[0])<16&&Math.abs(PolygonHolder.TempPolygon.get(j+1)[1]-PolygonHolder.TempPolygon.get(0)[1])<16)
                ...
                if (PolygonHolder.TempPolygon.size() == 2) System.out.println("Unable to create polygon from 2 points or unknown error.");
                else{
